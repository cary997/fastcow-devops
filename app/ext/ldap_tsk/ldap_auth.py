...
from app.models.system_model import LdapAttributesMap
...
class LdapAuthMixin:
    ...
    def __init__(
        ...
        password: str,
        base_ou: str,
        attributes: LdapAttributesMap = LdapAttributesMap(),
        paged_size: int = 500,
        **kwargs,  # pylint: disable=unused-argument
    ):
        """
        初始化ldap连接池
        :param hosts: 主机列表["ldap_tsk(s)://ldap_tsk.dc01.com:389","ldap_tsk(s)://ldap_tsk.dc02.com:389"]
        ...
        conn_pool = ServerPool(
            None, pool_strategy="ROUND_ROBIN", active=2, exhaust=True
        )
        try:
...
if __name__ == "__main__":
    test = LdapAuthMixin(
        hosts=["ldap://192.168.112.128:389"],
        user="CN=Administrator,CN=Users,DC=fastcow,DC=com",
        password="guo123.0",
    ...
    print(
        test.verify_user("CN=张三,OU=运维部,OU=fastapi,DC=fastcow,DC=com", "guo123.0")
    )
