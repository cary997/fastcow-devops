...
async def access_token(
        session: AsyncSessionDep, verify_info: Annotated[LoginVerifyDepends, Depends(user_login)]
) -> Any:
    """
    用户登陆
    """
    ...
    if totp_enable:
        ...
        try:
            ...
            if not verify_totp(user_totp, totp_code):
                return response(message="MFA验证失败!").fail(status_code=403)
        except ValueError:
            return response(message="无效令牌").fail(status_code=403)
...
async def refresh_token(session: AsyncSessionDep, post: schema.RefreshToken) -> Any:
    """
    刷新jwt
    """
    ...
    try:
        ...
        if verify_password(
            ...
            user = await session.get(Users, access_payload.get("user_id"))
            if not user:
                return response(message="用户不存在或已删除!").fail(status_code=403)
            if not user.user_status:
                return response(message="用户已被禁用!").fail(status_code=403)
            user_jwt = format_token(user)
            # jwt签发成功写入redis
            await set_redis_data(
                f"jwt:{user.id}",
...
@router.get(
    ...
    dependencies=[Security(check_token_dep)],
)
async def async_routes(session: AsyncSessionDep, request: Request) -> Any:
    """
    同步动态路由
    """
