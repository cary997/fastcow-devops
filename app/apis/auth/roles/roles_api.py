...
async def auth_roles_add(
    session: AsyncSessionDep, create_roles: schema.RoleCreate
) -> Any:
    response = schema.RoleCreateResponse
    stmt = select(Roles).where(Roles.name == create_roles.name)
    roles = (await session.exec(stmt)).first()
    if roles:
        return response(message=f"对象 {create_roles.name} 已经存在!").fail()
    add_role = await crud.create_role(session=session, role_create=create_roles)
    if not add_role:
...
async def auth_roles_del(session: AsyncSessionDep, role_id: int) -> Any:
    """
    删除角色
    """
...
async def auth_roles_set(
    session: AsyncSessionDep, role_id: int, update_role: schema.RoleUpdate
) -> Any:
    """
    更新角色
    """
