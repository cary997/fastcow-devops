...
async def auth_menus_add(session: SessionDep, create_menu: schema.MenuCreate) -> Any:
    """
    创建菜单
    """
...
async def auth_menus_del(session: SessionDep, menu_id: int) -> Any:
    """
    删除菜单
    """
...
async def auth_menus_set(
    session: SessionDep, menu_id: int, update_menu: schema.MenuUpdate
) -> Any:
    """
    更新菜单
    """
...
async def auth_menus_list(session: SessionDep, to_tree: bool = Query(True)) -> Any:
    """
    过滤菜单
    """
    response = schema.MenuQueryResponse
    # 查询结果
    query_data = session.exec(select(Menus)).all()
    # 格式化为树
    if to_tree:
